...
- **桌面通知** — 在用户多半不在面板前的时机主动弹 Windows toast：5 小时窗口将早于重置耗尽、auto-quota 切换账号、引擎暂停。原生 `Windows.UI.Notifications`，零第三方依赖，同事件 30 分钟去重限频，失败不影响请求路径；`COMMANDCODE_NOTIFY=0` 关闭
